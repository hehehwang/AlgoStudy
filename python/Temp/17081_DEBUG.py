...
fR, fC = map(int, f.readline().split())
field = [list(f.readline().strip()) for _ in range(fR)]
...
def events(c):
    ...
    if curr == 'B':
        ...
        print(itm)
        if itm[0] == 'O':
            if inven['accCnt'] == 4: pass
            else: inven['acc'][itm[1]] = True
            inven['accCnt'] += 1
...
def battle(hr, hc):
    ...
    while 1:
        ...
        if mhp <= 0:
            expGain(monster['exp'])
            if inven['acc']['HR']:
                stat['hp'] = max(stat['hp']+3, stat['hpMax'])
            field[hr][hc] = '.'
            if monster['name'] == 'Boss': youWin()
            break
...
def dead(hr, hc):
    ...
    if not inven['acc']['RE']:
        ...
        if field[hr][hc] == '^':
            ...
            print('YOU HAVE BEEN KILLED BY', mobs[hr][hc]['name'], '..')
    else:
        inven['acc']['RE'] = False
        heroCoord = heroInit[:]
        stat['hp'] = stat['hpMax']
...
def gameOver(n):
    ...
    for i in range(fR):
        for j in range(fC):
            if n and (i+1, j+1) == heroCoord: print('@', end='')
...
def display():
    global turns, heroCoord
    print(heroCoord)
    for i in range(fR):
        for j in range(fC):
            if i == heroCoord[0] and j == heroCoord[1]:
                print('@', end='')
            else: print(field[i][j], end='')
        print()
    print('Passed Turns :', turns)
    ...
    print(f"DEF : {stat['def']}+{inven['armor']}")
    print(f"EXP : {stat['exp']}/{stat['lv']*5}")
    print("Acc cnt :", inven['accCnt'])
    [print(acc, end=' ') for acc in inven['acc'].keys() if inven['acc'][acc]]
    print()
