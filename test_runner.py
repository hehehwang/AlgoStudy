...
def runner():
    ...
    for i in range(IDX_RANGE[0], IDX_RANGE[1] + 1):
        ...
        input_testfile = INPUT_DIR.joinpath(
            PREFIX + IO_SELECTOR[0] + idx + POSTFIX)
        answer_testfile = OUTPUT_DIR.joinpath(
            PREFIX + IO_SELECTOR[1] + idx + POSTFIX)

        if not os.path.isfile(input_testfile):
            raise Exception("Test input file not found in " +
                            str(input_testfile))
        if IS_OUTPUT and not os.path.isfile(answer_testfile):
            raise Exception("Test output file not found in " +
                            str(answer_testfile))
